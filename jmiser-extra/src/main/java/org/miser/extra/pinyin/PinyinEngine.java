...
public interface PinyinEngine {
	...
	default String getFirstLetter(String str, String separator) {
		...
		final List<String> split = StringUtil.split(getPinyin(str, splitSeparator), splitSeparator);
		return CollUtil.join(split, separator, (s)->String.valueOf(s.length() > 0 ? s.charAt(0) : StringUtil.EMPTY));
	}
}
