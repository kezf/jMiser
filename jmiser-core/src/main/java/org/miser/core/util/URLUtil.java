...
import org.miser.core.net.URLDecoder;
import org.miser.core.net.URLEncoder;
...
public class URLUtil {
	...
	public static URI getStringURI(CharSequence content) {
		final String contentStr = StringUtil.addPrefixIfNot(content, "string:///");
	...
	public static String encodeAll(String url, Charset charset) throws UtilException {
		if (null == charset || StringUtil.isEmpty(url)) {
			return url;
		}
	...
	/**
	 * Data URI Scheme封装，数据格式为Base64。data URI scheme
	 * 允许我们使用内联（inline-code）的方式在网页中包含数据，<br>
