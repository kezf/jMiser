...
import org.miser.core.annotation.AnnotationUtil;
import org.miser.core.annotation.Ignore;
...
public class PropertyDesc {
	...
	public Type getFieldType() {
		...
		return findPropertyType(getter, setter);
	}
	...
	public Class<?> getFieldClass() {
		...
		return findPropertyClass(getter, setter);
	}
	...
	public PropertyDesc setValue(Object bean, Object value, boolean ignoreNull, boolean ignoreError) {
		...
		// 当类型不匹配的时候，执行默认转换
		if (null != value) {
			final Class<?> propertyClass = getFieldClass();
			if (false == propertyClass.isInstance(value)) {
				value = Convert.convertWithCheck(propertyClass, value, null, ignoreError);
			}
		}
	 ...
	 * @return {@link Type}
	 */
	private Type findPropertyType(Method getter, Method setter) {
		Type type = null;
		if (null != getter) {
	 ...
	 * @return {@link Type}
	 */
	private Class<?> findPropertyClass(Method getter, Method setter) {
		Class<?> type = null;
		if (null != getter) {
