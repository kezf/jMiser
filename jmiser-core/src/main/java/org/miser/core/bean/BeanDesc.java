...
public class BeanDesc implements Serializable {
	 ...
	 * 属性Map
	 */
	private final Map<String, PropertyDesc> propertyMap = new LinkedHashMap<>();
	 ...
	 * @return 字段名-字段属性Map
	 */
	public Map<String, PropertyDesc> getPropertyMap(boolean ignoreCase) {
		return ignoreCase ? new CaseInsensitiveMap<>(1, this.propertyMap) : this.propertyMap;
	}
	...
	public Field getField(String fieldName) {
		final PropertyDesc desc = this.propertyMap.get(fieldName);
	...
	private BeanDesc init() {
		...
		for (Field field : ReflectUtil.getFields(this.beanClass)) {
			if (false == ModifierUtil.isStatic(field)) {
				...
				this.propertyMap.putIfAbsent(prop.getFieldName(), prop);
			}
		}
		return this;
	}
	...
	private PropertyDesc createProp(Field field, Method[] methods) {
		...
		if (null == prop.getter || null == prop.setter) {
			final PropertyDesc ignoreCase = findProp(field, methods, true);
			if (null == prop.getter) {
				prop.getter = ignoreCase.getter;
			}
			if (null == prop.setter) {
				prop.setter = ignoreCase.setter;
			}
		}
