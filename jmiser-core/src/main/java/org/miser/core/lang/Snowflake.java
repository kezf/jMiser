...
import org.miser.core.date.SystemClock;
...
public class Snowflake implements Serializable {
	...
	/**
	 * @param epochDate        初始化时间起点（null表示默认起始日期）,后期修改会导致id重复,如果要修改连workerId
	 *                         dataCenterId，慎用
	...
	public Snowflake(Date epochDate, long workerId, long dataCenterId, boolean isUseSystemClock, long timeOffset) {
		...
		if (workerId > MAX_WORKER_ID || workerId < 0) {
			throw new IllegalArgumentException(
					StringUtil.format("worker Id can't be greater than {} or less than 0", MAX_WORKER_ID));
		}
		if (dataCenterId > MAX_DATA_CENTER_ID || dataCenterId < 0) {
			throw new IllegalArgumentException(
					StringUtil.format("datacenter Id can't be greater than {} or less than 0", MAX_DATA_CENTER_ID));
		}
	...
	private long tilNextMillis(long lastTimestamp) {
		...
		if (timestamp < lastTimestamp) {
			...
			throw new IllegalStateException(StringUtil.format("Clock moved backwards. Refusing to generate id for {}ms",
					lastTimestamp - timestamp));
		}
		return timestamp;
	}
