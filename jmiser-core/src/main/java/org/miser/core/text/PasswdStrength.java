...
public class PasswdStrength {
	...
	public static int check(String passwd) {
		...
		if (len % 2 == 0) { // aaabbb
			...
			if (StringUtil.isCharEquals(part1) && StringUtil.isCharEquals(part2)) {
				level--;
			}
		}
		if (len % 3 == 0) { // ababab
		...
		if (StringUtil.isCharEquals(passwd)) {
			level = 0;
		}
	...
	/**
	 * Get password strength level, includes easy, midium, strong, very strong,
	 * extremely strong
	 *
	 * @param passwd 密码
	 * @return 密码等级枚举
	...
	/**
	 * Check character's type, includes num, capital letter, small letter and other
	 * character. 检查字符类型
	 *
	 * @param c 字符
	 * @return 类型
