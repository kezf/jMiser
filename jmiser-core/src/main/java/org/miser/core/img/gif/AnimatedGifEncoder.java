package org.miser.core.img.gif;
...
public class AnimatedGifEncoder {
	...
	/**
	 * 设置每一帧的间隔时间 Sets the delay time between each frame, or changes it for
	 * subsequent frames (applies to last frame added).
	 *
	 * @param ms 间隔时间，单位毫秒
	 */
	 ...
	 * frames. Since all colors are subject to modification in the quantization
	 * process, the color in the final palette for each frame closest to the given
	 * color becomes the transparent color for that frame. If exactMatch is set to
	 * true, transparent color index is search with exact match, and not looking for
	 * the closest one. May be set to null to indicate no transparent color.
	 *
	 * @param c          Color to be treated as transparent on display.
	 * @param exactMatch If exactMatch is set to true, transparent color index is
	 ...
	 * Since all colors are subject to modification in the quantization process, the
	 * color in the final palette for each frame closest to the given color becomes
	 * the background color for that frame. May be set to null to indicate no
	 * background color which will default to black.
	 *
	...
	/**
	 * Returns true if the exact matching color is existing, and used in the color
	 * palette, otherwise, return false. This method has to be called before
	 * finishing the image, because after finished the palette is destroyed and it
	...
	/**
	 * Returns index of palette exactly matching to color c or -1 if there is no
	 * exact matching.
	 *
	 * @param c Color
	 * @return index
	...
	protected int findExact(Color c) {
		...
		for (int index = 0; index < len; ++index) {
			int i = index * 3;
			// If the entry is used in colorTab, then check if it is the same exact color
			// we're looking for
