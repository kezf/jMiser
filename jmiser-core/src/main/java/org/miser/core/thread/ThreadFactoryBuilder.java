...
import org.miser.core.builder.Builder;
...
public class ThreadFactoryBuilder implements Builder<ThreadFactory> {
	...
	/**
	 * 设置线程名前缀，例如设置前缀为th-thread-，则线程名为th-thread-1之类。
	 *
	...
	public ThreadFactoryBuilder setPriority(int priority) {
		if (priority < Thread.MIN_PRIORITY) {
			throw new IllegalArgumentException(
					StringUtil.format("Thread priority ({}) must be >= {}", priority, Thread.MIN_PRIORITY));
		}
		if (priority > Thread.MAX_PRIORITY) {
			throw new IllegalArgumentException(
					StringUtil.format("Thread priority ({}) must be <= {}", priority, Thread.MAX_PRIORITY));
		}
