...
public class NumberChineseFormatter {
	 ...
	 * 使用混合数组提高效率和数组复用
	 **/
	private static final char[] DIGITS = { '零', '一', '壹', '二', '贰', '三', '叁', '四', '肆', '五', '伍', '六', '陆', '七', '柒',
			'八', '捌', '九', '玖' };

	/**
	 * 汉字转阿拉伯数字的
	 */
	private static final ChineseUnit[] CHINESE_NAME_VALUE = { new ChineseUnit(' ', 1, false),
			...
			new ChineseUnit('万', 10000, true), new ChineseUnit('亿', 100000000, true), };
	...
	public static String format(double amount, boolean isUseTraditional, boolean isMoneyMode) {
		...
		if (numFen != 0 || numJiao != 0) {
			if (numFen == 0) {
				chineseStr.append(isMoneyMode ? "元" : "点").append(numberToChinese(numJiao, isUseTraditional))
						.append(isMoneyMode ? "角" : "");
			} else { // “分”数不为 0
				if (numJiao == 0) {
					chineseStr.append(isMoneyMode ? "元零" : "点零").append(numberToChinese(numFen, isUseTraditional))
							.append(isMoneyMode ? "分" : "");
				} else {
					chineseStr.append(isMoneyMode ? "元" : "点").append(numberToChinese(numJiao, isUseTraditional))
							.append(isMoneyMode ? "角" : "").append(numberToChinese(numFen, isUseTraditional))
							.append(isMoneyMode ? "分" : "");
				}
			}
		} else if (isMoneyMode) {
			...
			if (num >= 0) {
				if (num == 0) {
					...
					throw new IllegalArgumentException(
							StringUtil.format("Bad number '{}{}' at: {}", chinese.charAt(i - 1), c, i));
				}
				// 普通数字
				number = num;
			} else {
				...
				if (null == unit) {
					// 出现非法字符
					throw new IllegalArgumentException(StringUtil.format("Unknown unit '{}' at: {}", c, i));
				}
