...
import org.miser.core.lang.Validator;
...
public class Ipv4Util {
	 ...
	 * IP与掩码的分割符
	 */
	public static final String IP_MASK_SPLIT_MARK = StringUtil.SLASH;
	...
	public static List<String> list(String ipRange, boolean isAll) {
		if (ipRange.contains(IP_SPLIT_MARK)) {
			// X.X.X.X-X.X.X.X
			final String[] range = StringUtil.splitToArray(ipRange, IP_SPLIT_MARK);
			return list(range[0], range[1]);
		} else if (ipRange.contains(IP_MASK_SPLIT_MARK)) {
			// X.X.X.X/X
			final String[] param = StringUtil.splitToArray(ipRange, IP_MASK_SPLIT_MARK);
	...
	public static List<String> list(String ip, int maskBit, boolean isAll) {
		...
		startIp = StringUtil.subPre(startIp, lastDotIndex)
				+ (Integer.parseInt(Objects.requireNonNull(StringUtil.subSuf(startIp, lastDotIndex))) + 1);
		lastDotIndex = endIp.lastIndexOf(CharUtil.DOT) + 1;
		endIp = StringUtil.subPre(endIp, lastDotIndex)
				+ (Integer.parseInt(Objects.requireNonNull(StringUtil.subSuf(endIp, lastDotIndex))) - 1);
		return list(startIp, endIp);
	}
	...
	public static List<String> list(String ipFrom, String ipTo) {
		final int[] ipf = Convert.convert(int[].class, StringUtil.splitToArray(ipFrom, CharUtil.DOT));
		final int[] ipt = Convert.convert(int[].class, StringUtil.splitToArray(ipTo, CharUtil.DOT));
	...
	public static long ipv4ToLong(String strIP) {
		...
		final long[] ip = Convert.convert(long[].class, StringUtil.split(strIP, CharUtil.DOT));
	...
	public static String getMaskByIpRange(String fromIp, String toIp) {
		...
		String[] fromIpSplit = StringUtil.splitToArray(fromIp, CharUtil.DOT);
		String[] toIpSplit = StringUtil.splitToArray(toIp, CharUtil.DOT);
	...
	public static int countByIpRange(String fromIp, String toIp) {
		...
		int count = 1;
		int[] fromIpSplit = StringUtil.split(fromIp, CharUtil.DOT).stream().mapToInt(Integer::parseInt).toArray();
		int[] toIpSplit = StringUtil.split(toIp, CharUtil.DOT).stream().mapToInt(Integer::parseInt).toArray();
