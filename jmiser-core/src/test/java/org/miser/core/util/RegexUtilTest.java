...
import org.miser.core.collection.CollectionUtil;
import org.miser.core.lang.PatternPool;
...
public class RegexUtilTest {
	...
	@Test
	public void getTest() {
		String resultGet = RegexUtil.get("\\w{2}", content, 0);
	...
	public void extractMultiTest() {
		// 抽取多个分组然后把它们拼接起来
		String resultExtractMulti = RegexUtil.extractMulti("(\\w)aa(\\w)", content, "$1-$2");
	...
	public void extractMultiTest2() {
		// 抽取多个分组然后把它们拼接起来
		String resultExtractMulti = RegexUtil.extractMulti("(\\w)(\\w)(\\w)(\\w)(\\w)(\\w)(\\w)(\\w)(\\w)(\\w)",
	...
	public void delFirstTest() {
		// 删除第一个匹配到的内容
		String resultDelFirst = RegexUtil.delFirst("(\\w)aa(\\w)", content);
	...
	public void delLastTest() {
		...
		// 空字符串兼容
		Assert.assertEquals(blank, RegexUtil.delLast("\\d+", blank));
		Assert.assertEquals(blank, RegexUtil.delLast(PatternPool.NUMBERS, blank));

		// 去除数字
		Assert.assertEquals("公斤", RegexUtil.delLast("\\d+", word));
		Assert.assertEquals("公斤", RegexUtil.delLast(PatternPool.NUMBERS, word));
		// 去除汉字
		Assert.assertEquals("180", RegexUtil.delLast("[\u4E00-\u9FFF]+", word));
		Assert.assertEquals("180", RegexUtil.delLast(PatternPool.CHINESES, word));

		// 多个匹配删除最后一个 判断是否不在包含最后的数字
		String s = RegexUtil.delLast("\\d+", sentence);
		Assert.assertEquals("10.商品KLS100021型号xxl适合身高180体重斤的用户", s);
		s = RegexUtil.delLast(PatternPool.NUMBERS, sentence);
		Assert.assertEquals("10.商品KLS100021型号xxl适合身高180体重斤的用户", s);

		// 多个匹配删除最后一个 判断是否不在包含最后的数字
		Assert.assertFalse(RegexUtil.delLast("[\u4E00-\u9FFF]+", sentence).contains("斤的用户"));
		Assert.assertFalse(RegexUtil.delLast(PatternPool.CHINESES, sentence).contains("斤的用户"));
	}
	...
	public void delAllTest() {
		...
		String resultDelAll = RegexUtil.delAll("!\\[images\\][^\\u4e00-\\u9fa5\\\\s]*", content);
	...
	public void findAllTest() {
		// 查找所有匹配文本
		List<String> resultFindAll = RegexUtil.findAll("\\w{2}", content, 0, new ArrayList<>());
	...
	public void getFirstNumberTest() {
		// 找到匹配的第一个数字
		Integer resultGetFirstNumber = RegexUtil.getFirstNumber(content);
	...
	public void isMatchTest() {
		// 给定字符串是否匹配给定正则
		boolean isMatch = RegexUtil.isMatch("\\w+[\u4E00-\u9FFF]+\\d+", content);
	...
	public void replaceAllTest() {
		...
		// 此处把1234替换为 ->1234<-
		String replaceAll = RegexUtil.replaceAll(content, "(\\d+)", "->$1<-");
	...
	public void replaceAllTest2() {
		// 此处把1234替换为 ->1234<-
		String replaceAll = RegexUtil.replaceAll(this.content, "(\\d+)",
	...
	public void getAllGroupsTest() {
		...
		List<String> allGroups = RegexUtil.getAllGroups(pattern, "192-168-1-1");
		...
		allGroups = RegexUtil.getAllGroups(pattern, "192-168-1-1", false);
