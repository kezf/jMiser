...
import org.miser.core.map.MapBuilder;
...
public class XmlUtilTest {
	...
	public void mapToXmlTest() {
		Map<String, Object> map = MapBuilder.create(new LinkedHashMap<String, Object>())//
				...
				.put("age", 12)//
				.put("game",
						MapUtil.builder(new LinkedHashMap<String, Object>()).put("昵称", "Looly").put("level", 14)
								.build())//
				.build();
	...
	public void mapToXmlTest2() {
		// 测试List
		Map<String, Object> map = MapBuilder.create(new LinkedHashMap<String, Object>())
				.put("Town", CollUtil.newArrayList("town1", "town2")).build();

		Document doc = XmlUtil.mapToXml(map, "City");
		Assert.assertEquals("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>" + "<City>"
				+ "<Town>town1</Town>" + "<Town>town2</Town>" + "</City>", XmlUtil.toStr(doc));
	}
	...
	public void readBySaxTest() {
		final Set<String> eles = CollUtil.newHashSet("returnsms", "returnstatus", "message", "remainpoint", "taskID",
				"successCounts");
	...
	public void mapToXmlTestWithOmitXmlDeclaration() {
		...
		Map<String, Object> map = MapBuilder.create(new LinkedHashMap<String, Object>()).put("name", "ddatsh").build();
	...
	public void getByPathTest() {
		...
		Object value = XmlUtil.getByXPath("//soap:Envelope/soap:Body/ns2:testResponse/return", document,
	...
	public void cleanCommentTest() {
		final String xmlContent = "<info><title>test</title><!-- 这是注释 --><lang>java</lang></info>";
		final String ret = XmlUtil.cleanComment(xmlContent);
		Assert.assertEquals("<info><title>test</title><lang>java</lang></info>", ret);
	}
