...
import org.miser.poi.excel.cell.FormulaCellValue;
...
public class SheetDataSaxHandler extends DefaultHandler {
	...
	// 上一次的内容
	private final FastStringBuilder lastContent = StringUtil.fastBuilder();
	// 上一次的内容
	private final FastStringBuilder lastFormula = StringUtil.fastBuilder();
	// 存储每行的列元素
	...
	private void endCell() {
		...
		if (false == this.lastFormula.isEmpty()) {
			value = new FormulaCellValue(StringUtil.trim(lastFormula), value);
		}
	...
	private void fillBlankCell(String preCoordinate, String curCoordinate, boolean isEnd) {
		if (false == curCoordinate.equals(preCoordinate)) {
			...
			while (len-- > 0) {
				addCellValue(curCell++, StringUtil.EMPTY);
			}
		}
	}
