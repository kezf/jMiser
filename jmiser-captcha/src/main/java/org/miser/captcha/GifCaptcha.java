...
import org.miser.core.img.gif.AnimatedGifEncoder;
...
public class GifCaptcha extends AbstractCaptcha {
	...
	private BufferedImage graphicsImage(char[] chars, Color[] fontColor, char[] words, int flag) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			g2d.drawOval(RandomUtil.randomInt(width), RandomUtil.randomInt(height), RandomUtil.randomInt(5, 30),
					5 + RandomUtil.randomInt(5, 30));// 绘制椭圆边框
	...
	private Color getRandomColor(int min, int max) {
		...
		return new Color(RandomUtil.randomInt(min, max), RandomUtil.randomInt(min, max),
