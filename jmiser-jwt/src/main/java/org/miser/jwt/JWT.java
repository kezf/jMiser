...
import org.miser.jwt.signers.AlgorithmUtil;
import org.miser.jwt.signers.JWTSigner;
import org.miser.jwt.signers.JWTSignerUtil;
import org.miser.jwt.signers.NoneJWTSigner;
...
public class JWT implements RegisteredPayload<JWT>{
	...
	public String sign(JWTSigner signer) {
		...
		return StringUtil.format("{}.{}.{}", headerBase64, payloadBase64, sign);
	}
	...
	private static List<String> splitToken(String token) {
		final List<String> tokens = StringUtil.split(token, CharUtil.DOT);
		if (3 != tokens.size()) {
