...
import org.miser.core.thread.GlobalThreadPool;
import org.miser.core.util.StringUtil;
import org.miser.http.server.action.Action;
import org.miser.http.server.action.RootAction;
import org.miser.http.server.filter.HttpFilter;
import org.miser.http.server.filter.SimpleFilter;
import org.miser.http.server.handler.ActionHandler;
...
public class SimpleServer {
	...
	public HttpContext createContext(String path, HttpHandler handler) {
		// 非/开头的路径会报错
		path = StringUtil.addPrefixIfNot(path, StringUtil.SLASH);
	...
	public void start() {
		...
		Console.log("Simple Http Server listen on 【{}:{}】", address.getHostName(), address.getPort());
		this.server.start();
	}
}
